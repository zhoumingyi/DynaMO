...
def get_attributes_params(op, interpreter, adv_value=1.0, act_obf=None, only_weights_value=1.0, only_bias_value=1.0):
    kwargs = {}
    if op['builtin_options_type'] == 'Conv2DOptions' or op['builtin_options_type'] == 'DepthwiseConv2DOptions':
        ...
        # load the activation
        if act_obf != None:
            kwargs['activation='] = 'activation=' + conv_activation_parser(act_obf)
        else:
            try:
        ...
        for tensor_details in interpreter.get_tensor_details():
            if tensor_details['index'] == op['inputs'][1]:
                # print("index: ", tensor_details['index'])
                filter_tensor = interpreter.get_tensor(tensor_details["index"]) * adv_value * only_weights_value
                # print("shape", filter_tensor.shape)
                ...
                bias_tensor = interpreter.get_tensor(tensor_details["index"]) * adv_value * only_bias_value
        ...
        if act_obf == "RELU6":
            kwargs['act_max=99.0'] = 'act_max=6.0'
            kwargs['act_min=-99.0'] = 'act_min=0.0'
        elif act_obf == "RELU":
            kwargs['act_max=99.0'] = 'act_max=FLT_MAX'
            kwargs['act_min=-99.0'] = 'act_min=-FLT_MAX'
        elif act_obf == None:
            kwargs['act_max=99.0'] = 'act_max=FLT_MAX'
            kwargs['act_min=-99.0'] = 'act_min=-FLT_MAX'
        else:
            raise ValueError("fused activation function not found, Op sign: " + str(op['sign']))
        kwargs['obf_value=1.0'] = 'obf_value=' + str(adv_value*only_weights_value)
    return kwargs
...
def correct_json(interpreter, model_json):
    ...
    for i in range(len(json_op_details)):
        ...
        if 'builtin_options_type' in json_op_details[i]:
            # print("model_json: ", json_op_details[i]['builtin_options_type'])
            # print("interpreter: ", op_details[i]['op_name'])
...
def lib_generator(model_json, interpreter, inout_list, injected_pair={}, adv_obf_value={}, activation_type={}, intermediate_nodes={}):
    ...
    for op in json_op_details:
        ...
        for i in intermediate_nodes.keys():
            for j in range(len(intermediate_nodes[i])):
                if op['sign'] == intermediate_nodes[i][j]:
        ...
        kwargs = get_attributes_params(op, interpreter, adv_value, act_obf, only_weights_value, only_bias_value)
